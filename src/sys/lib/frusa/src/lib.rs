...
//! returned by an explicit `reclaim()` call. Owner lookup and free-slot
//! search are bounded (a sorted block index and a partial stack), no lock
//! is held across a backend call, and callers may hold a per-thread cache
//! of private blocks. The design is in `docs/frusa.md`.
