...
pub struct PacketMeta {
    ...
    /// TCP segmentation offload: if nonzero, this packet is a TCP
    /// "super-segment" whose payload exceeds the connection's effective
    /// MSS, and the device must segment it on the wire into TCP segments
    /// carrying at most `tso_seg_size` payload bytes each (e.g. virtio's
    /// VIRTIO_NET_HDR_GSO_TCPV4 with gso_size = tso_seg_size).
    ///
...
pub struct DeviceCapabilities {
    ...
    /// TCP segmentation offload: the maximum TCP payload size, in bytes, the
    /// device accepts in a single transmitted TCP "super-segment" (it will
    /// segment such packets into MSS-sized wire segments itself, using the
    /// per-packet [`PacketMeta::tso_seg_size`] as the segment payload size).
    /// 0 disables TSO: TCP sockets never emit segments above the effective
    /// MSS. Devices advertising a nonzero value must also handle the
    /// resulting `consume(len)` calls with `len` up to this value plus IP,
    /// TCP and link-layer headers, and typically want
    /// [`ChecksumCapabilities::tcp`] TX offload as well (the device or its
    /// host completes per-segment checksums).
    ///
    /// This affects transmission only; it does not change the MSS advertised
    /// to the remote peer, so received packets stay MTU-bounded.
