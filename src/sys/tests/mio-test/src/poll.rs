...
fn test_run_once_with_nothing() {
    ...
    poll.poll(&mut events, Some(Duration::from_millis(100)))
        .unwrap();
    println!("poll::test_run_once_with_nothing PASS");
}

fn test_add_then_drop() {
    init();
    ...
    poll.poll(&mut events, Some(Duration::from_millis(100)))
        .unwrap();
    println!("poll::test_add_then_drop PASS");
}

fn test_zero_duration_polls_events() {
    init();
    ...
    drop(streams);
    drop(listener);
    println!("poll::test_zero_duration_polls_events PASS");
}
...
fn test_poll_closes_fd() {
    ...
    println!("poll::test_poll_closes_fd PASS");
}
...
fn test_drop_cancels_interest_and_shuts_down() {
    ...
    drop(stream);
    handle.join().unwrap();
    println!("poll::test_drop_cancels_interest_and_shuts_down PASS");
}
...
fn test_registry_behind_arc() {
    ...
    handle1.join().unwrap();
    handle2.join().unwrap();
    println!("poll::test_registry_behind_arc PASS");
}
...
fn test_registry_operations_are_thread_safe() {
    ...
    */
    ...
    // handle2.join().unwrap();
    handle3.join().unwrap();
    println!("poll::test_registry_operations_are_thread_safe PASS");
}

fn test_register_during_poll() {
    let (mut poll, mut events) = init_with_poll();
    ...
    let handle1 = thread::spawn(move || {
        let mut stream = TcpStream::connect(any_local_address()).unwrap();
    ...
    barrier.wait();
    handle1.join().unwrap();
    println!("poll::test_register_during_poll PASS");
}
...
pub fn test_double_register_different_token() {
    init();
...
pub fn run_all_tests() {
    ...
    test_run_once_with_nothing();
    test_add_then_drop();
    test_zero_duration_polls_events();
    test_poll_closes_fd();
