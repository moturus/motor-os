...
impl SelfHost {
    ...
    fn new(name: &str, scripts: Vec<Script>) -> SelfHost {
        ...
        // Cargo reads every target's manifest even when building for the host.
        // Point Motor-only path dependencies back at the real tree rather
        // than copying half the OS into the self-host fixture.
        let mut manifest = std::fs::read_to_string(work.join("Cargo.toml")).unwrap();
        for dependency in ["moto-rt", "moto-sys"] {
            let relative = format!("../../sys/lib/{dependency}");
            let absolute = crate_dir().join(&relative).canonicalize().unwrap();
            manifest = manifest.replace(&relative, absolute.to_str().unwrap());
        }
