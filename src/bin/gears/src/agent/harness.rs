//! Assembly: one workspace, one session, one agent, one thread.
//!
...
impl Harness {
    pub fn start(
        ...
        let mut bus = Bus::new(ROOT, event_tx);
        ...
        let thread = std::thread::spawn(move || {
            while let Ok(Command::Prompt(text)) = command_rx.recv() {
                ...
                if bus
                    .turn_end(agent.usage(), outcome == Turned::Done)
                    .is_err()
                    || outcome == Turned::Gone
                {
                    break;
...
mod tests {
    ...
    use crate::provider::{ChatRequest, Completion, EventSink, FinishReason, ProviderError, Usage};
    use std::sync::Mutex;
    ...
    fn answers(texts: &[&str]) -> Box<dyn ModelProvider + Send> {
        Box::new(Fixed(Mutex::new(
            texts
                .iter()
                .rev()
                .map(|text| Completion {
