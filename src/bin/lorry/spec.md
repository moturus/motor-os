...
Cargo or Lorry. Lorry owns `.lorry/dependencies-v1.toml`; it is deterministic,
portable, intended to be committed, and must be changed only by Lorry.

The generated state is evidence about a resolved registry graph, never an additional
version requirement. Format 1 contains only:

- a canonical semantic fingerprint of Cargo.lock registry nodes and their
  recorded dependency edges;
- the exact registry identities in Cargo.lock, including inactive nodes;
- the target triples whose selected closures were admitted;
- for every admitted registry source, its exact package identity, checksum,
  license, source-tree digest, and build-script presence;
- the build-script and native-tool capabilities approved for that identity;
  and
- the Lorry state and source-tree format versions.

The locked registry list and selected admitted list are distinct. Recording an
inactive Cargo.lock node does not claim its source was acquired or admitted.
