            ...
            match data[0].result {
                0 => {
                    ...
                    for _ in 0..num_entries {
                        ...
                        if pos >= buffer.len() {
                            moto_sys::syscalls::SysMem::log("bad tail entries response (1)").ok();
                            return Err(ErrorCode::InternalError);
                        }
                        let curr_buffer = &buffer[pos..];
                        if curr_buffer.len() < size_of::<LogEntryHeader>() {
                            moto_sys::syscalls::SysMem::log("bad tail entries response (2)").ok();
                            return Err(ErrorCode::InternalError);
                        }
                        ...
                        if pos + size_of::<LogEntryHeader>() + (header.payload_size as usize)
                            > buffer.len()
                        {
                            moto_sys::syscalls::SysMem::log("bad tail entries response (3)").ok();
                            return Err(ErrorCode::InternalError);
                        }
