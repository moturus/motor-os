...
| `mm/kheap.rs` | The kernel heap: `frusa` with a per-CPU stage (private blocks and a magazine of freed slots) over a page supplier that bumps from the boot heap until memory is initialized, then allocates from the kernel heap region. |
