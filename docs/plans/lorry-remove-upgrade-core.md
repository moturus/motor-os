...
Each implementation patch will use `test-changed.sh --print` and the selected
Lorry-local gate. No repository-wide OS test is required unless a patch grows
beyond Lorry-owned code or its integration drivers.
