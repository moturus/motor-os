...
With every other implementation step committed and gated, bounded diagnostic
cases isolate the queued hover. These are investigations, not replacement
acceptance gates; the maintained whole-case bound remains 90 seconds.

The diagnostic fixture compares integer, plain-string, generated-integer, and
`env!`-derived string constants with the same selected analyzer revision and
Motor target. Linux initially uses Cargo; Motor uses Lorry and the packaged
native server. Linux results are:

- workspace readiness: 5.659 seconds;
- integer hover: 1.933 ms;
- first plain-string hover: 870.377 ms;
- generated-integer hover: 0.878 ms;
- subsequent environment-string hover: 16.395 ms.

Motor reaches readiness in 18.396 seconds and answers the integer hover in
1.946 ms, but the plain-string hover exhausts the original whole-case bound.
Thus the failure is not specific to `env!` expansion or build-script data.
Forcing Linux to use a diagnostic rust-src copy without its workspace
manifest selects the same ten-crate stitched sysroot used on Motor. That
plain-string hover still completes in 576.699 ms. The sysroot-loading
difference does not explain the native timeout.

The source path is `hir::EvaluatedConst::render_debug` ->
`mir::render_const_using_debug_impl` -> MIR/type inference for
`std::fmt::format`. Integers already have a direct rendering path; strings
take the interpreted `Debug` path. A separate bounded native diagnostic uses
the existing `infer_query` profiler and two existing-debugger stack captures,
resuming the process immediately after each capture. The worker is actively
building syntax/AST maps while collecting inherent implementations for method
lookup. This is not the earlier child-cleanup deadlock.

The second capture places both busy threads inside the runtime allocator:
the hover worker in small-allocation block traversal, and `ParseNodeDropper`
in deallocation block traversal. The packaged vDSO is stripped, so this
identification uses its disassembly, not invented symbolized frames:
offset `0x7a564` is the allocation list walk and `0x7abc0` is the deallocation
range test/list walk. Their instructions correspond to
`src/sys/lib/frusa/src/lib.rs`, `Slab::alloc` and `Slab::dealloc`. Both search
from the list head; cost can grow with the number of blocks in a size class.

A separate release-mode Linux diagnostic calls the unmodified Motor Frusa4K
directly, with Linux `System` as its backing allocator, to isolate the
algorithm from Motor syscalls, networking, and rust-analyzer. It retains
64-byte allocations, repeatedly frees/replaces the oldest live allocation
10,000 times, then frees all retained allocations. Its pointer bookkeeping
uses the host allocator and is outside the allocation measurements.
...
These measurements establish an existing allocator scalability problem on
the observed hover path. They do not yet quantify its share of total hover
latency or prove that an allocator correction alone meets the native bound;
that requires an end-to-end before/after comparison. No allocator, runtime,
stdlib, external Rust tree, timeout, or maintained assertion was changed by
this investigation. Diagnostic source and evidence are retained under
`/tmp/motor-ra-hover.qAVs4I/`: `linux-all.log`, `motor-all.log`,
`linux-stitched.log`, the corresponding `stderr` files,
`profile-stacks-{1,2}.log`, `profile-stack-2-symbols.log`, and `allocator.log`.

**Review requested:** approve a separately reviewed allocator optimization
plan, focused on `src/sys/lib/frusa`, before implementation. Address scalable
block lookup/free-space selection while preserving allocation correctness,
concurrency, and reclamation; do not simply increase cache sizes or bypass
string formatting to hide this problem. Any broader runtime change requires
its own explanation. This is the `AGENTS.md` stop for a pre-existing core
problem and a non-trivial design decision, not completion of Stage 2.

After review, add deterministic allocator regression coverage transitively
to `src/tests/full-test.sh`, validate the algorithm with the isolated workload
and native hover, and run the core debug/release gates three times each plus
`src/tests/full-test-dev.sh --release`. Restore the queued string-hover
assertion and meet its existing whole-case/resource limits before converting
this plan to maintained documentation.
